<?php
	class Winnowingmodel3 extends CI_Model{
        ...
        public function getselect($selected){
            ...
            $query = $this->db->query("SELECT `nama`, `judul`, `text`, `date_upload` FROM `filebankwinnowing` WHERE `id` = ?", array($selected));
        ...
        public function showdata(){
            ...
            $this->db->from('filebankwinnowing');
            $query = $this->db->get();
            $this->db->last_query();
		...
		public function uploadData($data){
			...
			return $this->db->insert('filebankwinnowing', $data);
		}
